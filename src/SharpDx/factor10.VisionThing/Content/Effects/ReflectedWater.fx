...
float DoDispMapping(float2 texC0, float2 texC1)
{
	...
	float2 texelpos = DMAP_SIZE * texC0;
		...
		/*
		float dmap0[4];
		dmap0[0] = WaveDispMap0.SampleLevel(MySampler2, texC0, 0).r;
		dmap0[1] = WaveDispMap0.SampleLevel(MySampler3, texC0 + float2(DMAP_DX, 0.0f), 0).r;
		dmap0[2] = WaveDispMap0.SampleLevel(MySampler3, texC0 + float2(0.0f, DMAP_DX), 0).r;
		dmap0[3] = WaveDispMap0.SampleLevel(MySampler3, texC0 + float2(DMAP_DX, DMAP_DX), 0).r;
		...
		float dmap1[4];
		dmap1[0] = WaveDispMap1.SampleLevel(MySampler3, texC1, 0).r;
		dmap1[1] = WaveDispMap1.SampleLevel(MySampler3, texC1 + float2(DMAP_DX, 0.0f), 0).r;
		dmap1[2] = WaveDispMap1.SampleLevel(MySampler3, texC1 + float2(0.0f, DMAP_DX), 0).r;
		dmap1[3] = WaveDispMap1.SampleLevel(MySampler3, texC1 + float2(DMAP_DX, DMAP_DX), 0).r;
...
{
    float4 bumpColor = BumpMap0.Sample(MySampler, input.BumpSamplingPos0);
	...
	bumpColor = bumpColor * bumpMapDistribution + BumpMap1.Sample(MySampler, input.BumpSamplingPos1) * (1-bumpMapDistribution);
    ...
    float4 reflectiveColor = ReflectedMap.Sample(MySampler, perturbatedTexCoords);
