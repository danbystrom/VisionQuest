    ...
    float2 scaledTexC     : TEXCOORD1)
{
	...
	posLocal.y = 0; //lerp(DoDispMapping(vTex0, vTex1), 0.75, lakeBlendFactor);
	...
	float r = 0; //DoDispMapping(vTex0 + float2(DMAP_DX, 0.0f), vTex1 + float2(0.0f, DMAP_DX));
	float b = 0; //DoDispMapping(vTex0 + float2(DMAP_DX, 0.0f), vTex1 + float2(0.0f, DMAP_DX));
