                 ...
                 float2 normalizedTexC : TEXCOORD1)
{
	...
	float3 lightDirL = mul(float4(gLightDirW, 0.0f), gWorldInv).xyz;
...
OutputVS WaterVS_Distant(float3 posL           : POSITION0, 
                 float2 scaledTexC     : TEXCOORD0,
                 float2 normalizedTexC : TEXCOORD1)
{
    // Zero out our output.
	...
	// Transform to-eye vector to tangent space.
	float3 toEyeL = eyePosL - posL;
	outVS.toEyeT = float3( toEyeL.x, -toEyeL.z, toEyeL.y );
	
	// Transform light direction to tangent space.
	float3 lightDirL = mul(float4(gLightDirW, 0.0f), gWorldInv).xyz;
	outVS.lightDirT  = float3( lightDirL.x, -lightDirL.z, lightDirL.y );
               ...
               float2 tex1      : TEXCOORD3) : COLOR
{
	...
	float t  = pow(max(dot(r, toEyeT), 0.0f), gMtrlSpecPower);
...
technique DistantWaterTech
{
    pass P0
    {
        // Specify the vertex and pixel shader associated with this pass.
        vertexShader = compile vs_3_0 WaterVS_Distant();
        pixelShader  = compile ps_3_0 WaterPS();
    }    
}
